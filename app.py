...
# Painel Administrativo com funcionalidades futuras
    ...
    for u in usuarios:
        ...
        projetos_selecionados = st.multiselect(f"Projetos ({user})", options=todos_projetos, default=projetos_atuais, key=f"projetos_{user}")
            permissoes = st.multiselect(f"Permissões ({user})", options=["upload", "download", "view"], default=permissoes_atuais.split(',') if permissoes_atuais else [], key=f"perms_{user}")
        if st.button(f"Atualizar senha {user}"):
                c.execute("UPDATE users SET password=?, projects=?, permissions=? WHERE username=?", (nova_senha, ','.join(projetos_selecionados), ','.join(permissoes), user))
                conn.commit()
